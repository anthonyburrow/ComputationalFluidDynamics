...
class NonlinearConvection(Model):
    ...
    def _upstream_step(self):
        ...
        self.u[1:, 1:] -= \
            self.C[0] * self.u[1:, 1:] * (self.u[1:, 1:] - self.u[1:, :-1]) + \
            self.C[1] * self.v[1:, 1:] * (self.u[1:, 1:] - self.u[:-1, 1:])

        self.v[1:, 1:] -= \
            self.C[0] * self.u[1:, 1:] * (self.v[1:, 1:] - self.v[1:, :-1]) + \
            self.C[1] * self.v[1:, 1:] * (self.v[1:, 1:] - self.v[:-1, 1:])
...
params = {
    ...
    'plot_fn': './NonlinearConvection.pdf',     # Output plot filename
    'animate': True,                            # Save frames as animation
    'animation_frames_to_skip': 5,              # Save every nth frame
    'gif_fn': './NonlinearConvection.gif',      # Output gif filename
}


if __name__ == '__main__':
    model = NonlinearConvection(params)
